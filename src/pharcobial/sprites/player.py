...
from pharcobial.types import UserInput, SpriteID
...
class Player(Character):
    ...
    def update(self, *args, **kwargs):
        ...
        new_x = round(self.hitbox.x + self.controller.direction.x * self.speed)
        new_y = round(self.hitbox.y + self.controller.direction.y * self.speed)

        self.move((new_x, new_y))
        flip = self.controller.right_focused
        bubble = self.graphics.get(Graphics.CHAT_BUBBLE, flip_vertically=flip)
        self.chat_bubble.image = bubble or self.chat_bubble.image

    def _get_graphic(self) -> Surface:
        if not self.moving:
