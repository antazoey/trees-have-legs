...
class SpriteManager(BaseManager):
    ...
    def player(self) -> Player:
        ...
        character.x = self.display.window_width // 2
        character.y = self.display.window_height // 2
    ...
    def create_adversary(self, type_key: str, **kwargs) -> Adversary:
        if type_key == "bush-monster":
            ...
            monster.x = random.randrange(20, self.display.window_width - BLOCK_SIZE - 10, 10)
            monster.y = random.randrange(20, self.display.window_height - BLOCK_SIZE - 10, 10)
            return monster
